...
class PICKDataset(Dataset):
    ...
    def __init__(self, files_name: str = None,
        ...
        if self.training:
            ...
            self.files_list = list(self.boxes_and_transcripts_folder.glob('*.json'))
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            entities_file = self.entities_folder.joinpath(Path(dataitem['file_name']).stem + '.json')
...
class BatchCollateFn(object):
    ...
    def __call__(self, batch_list: List[Document]):
        ...
        boxes_coordinate_padded_list = [F.pad(torch.FloatTensor(np.array(x.boxes_coordinate)),
                                              (0, 0, 0, max_boxes_num_batch - np.array(x.boxes_num)))
                                        for i, x in enumerate(batch_list)]
