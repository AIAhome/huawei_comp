...
from model.PICK.utils.entities_list import Entities_list
from model.PICK.utils.class_utils import keys_vocab_cls, iob_labels_vocab_cls, entities_vocab_cls
    ...
    def __init__(self, boxes_and_transcripts_file: Path, image_file: Path,
        ...
        try:
            ...
            if self.training:
                ...
                boxes_and_transcripts_data = read_ocr_file_CSIG(boxes_and_transcripts_file.as_posix())
            else:
                boxes_and_transcripts_data = read_ocr_file_CSIG(
...
def read_ocr_file_CSIG(filepath: str):
    with open(filepath, 'r', encoding='utf-8') as f:
        data = json.load(f)
...
def sort_box_with_list(data: List[Tuple], left_right_first=False):
    def compare_key(x):
        ...
        points = x[1]
        box = np.array(points, dtype=np.float32).reshape((4,2))
...
def text2iob_label_with_document_level_exactly_match(transcripts: List[str], entities: Dict) -> List[
    List[str]]:
    '''
    ...
    for entity_type, entity_value in exactly_entities_label:
        ...
        if tgt_len == 0:
            continue
        ##################################################################################################
        #               英文小票存在价格项ocr_labels和ie_labels标注不一。ie_labels为人工标注，带两位小数。
        #               ocr_labels为ocr识别的小票信息，由于小票信息有时为一位小数或无小数
        #               导致两个小票无法match
        ##################################################################################################
        for i in range(src_len - tgt_len + 1):
            if tgt_seq[0] == '$' and src_seq[i] == '$':  
                if src_seq[i+1 : i+tgt_len] == tgt_seq[1:]:  #两个label均为两位小数的情况
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i] + 1)
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i]:src_idx[i + tgt_len - 1] + 1] = tag
                elif src_seq[i+1 : i+tgt_len-1]+['0'] == tgt_seq[1:]:   #ocr_labels为一位小数的情况
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i])
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i]:src_idx[i + tgt_len - 1]] = tag
                elif src_seq[i+1 : i+tgt_len-3]+['.','0','0'] == tgt_seq[1:]:    # ocr_labels没有小数的情况
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i]-2)
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i]:src_idx[i + tgt_len - 1] -2] = tag
            ##################################################################################################
            #           英文date
            #           比如 09/16/2018或11'26'21
            ##################################################################################################
            if entity_type == 'date' and src_seq[i] == '/' and src_seq[i+3] == '/':
                eng_date1 = src_seq[i-2 : i]
                eng_date2 = src_seq[i+1 : i+3]
                eng_date3 = src_seq[i+4 : i+6]
                eng_date4 = src_seq[i+4 : i+8]
                datelist1 = eng_date4 + eng_date1
                datelist1.extend(eng_date2)
                datelist2 = ['2','0']
                datelist2.extend(eng_date3)
                datelist2.extend(eng_date1)
                datelist2.extend(eng_date2)
                datelist3 = ['2','0']
                datelist3.extend(eng_date3)
                datelist3.extend(eng_date2)
                datelist3.extend(eng_date1)
                datelist4 = eng_date4 + eng_date2
                datelist4.extend(eng_date1)

                if datelist1 == tgt_seq[0:]:
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i] + 3)
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i-2]:src_idx[i + tgt_len - 1] + 1] = tag
                if datelist2 == tgt_seq[0:]:
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i] + 1)
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i-2]:src_idx[i + tgt_len - 1] -1] = tag
                if datelist3 == tgt_seq[0:]:
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i] + 1)
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i-2]:src_idx[i + tgt_len - 1] -1] = tag
                if datelist4 == tgt_seq[0:]:
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i] + 3)
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i-2]:src_idx[i + tgt_len - 1] + 1] = tag
            if entity_type == 'date' and src_seq[i] == '\'' and src_seq[i+3] == '\'':
            ...
            ##################################################################################################
            #            中文date
            #            比如 2022 3 15 ，月份缺少一个0
            ##################################################################################################
            if entity_type == 'date' and src_seq[i] == '2' and src_seq[i+1] == '0':
                datelist = src_seq[i : i+4] + ['0']
                datelist.extend(src_seq[i+4:i+7])
                if datelist == tgt_seq[0:]:
                    tag = ['I-{}'.format(entity_type)] * (src_idx[i + tgt_len - 1] - src_idx[i])
                    tag[0] = 'B-{}'.format(entity_type)
                    result_tags[src_idx[i]:src_idx[i + tgt_len - 1]] = tag
            ##################################################################################################
